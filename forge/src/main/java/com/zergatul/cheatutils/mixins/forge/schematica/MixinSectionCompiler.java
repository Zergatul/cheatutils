...
public abstract class MixinSectionCompiler {
    ...
    private void onBeforeSorting(
            ...
            @LocalVariable(ordinal = 0) Map<BlockPos, ModelData> modelDataMap,
            @LocalVariable(ordinal = 1) Map<RenderType, BufferBuilder> map,
        ...
        for (BlockPos pos : BlockPos.betweenClosed(corner1, corner2)) {
            ...
            if (state.getRenderShape() == RenderShape.MODEL) {
                ...
                for (RenderType layer : model.getRenderTypes(state, random, data)) {
                    ...
                    this.blockRenderer.renderBatched(state, pos, wrapped, poseStack, consumer, true, random, data, layer);
                    poseStack.popPose();
                }
            }
        }
