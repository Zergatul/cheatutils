...
public class Privacy implements Module {
    ...
    public static final String EXPLOIT_PREVENTER_MOD_ID = "exploitpreventer";

    private Privacy() {}

    public boolean shouldDisconnectOnTranslationExploit(PrivacyConfig config) {
        return config.disconnectOnTranslationExploit && !ModEnvironment.hasMod(EXPLOIT_PREVENTER_MOD_ID);
    }

    public boolean shouldDisconnectOnMaliciousResourcePack(PrivacyConfig config) {
        return config.disconnectOnMaliciousResourcePackBehavior && !ModEnvironment.hasMod(EXPLOIT_PREVENTER_MOD_ID);
    }
