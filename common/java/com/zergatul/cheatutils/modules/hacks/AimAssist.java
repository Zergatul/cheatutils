...
public class AimAssist implements Module {
    ...
    private void onRenderTickStart(RenderTickStartEvent event) {
        ...
        if (targetLockEntity == null) {
            Rotation rotation = new Rotation(mc.player.getViewXRot(partialTicks), mc.player.getViewYRot(partialTicks));
    ...
    private @Nullable Entity findTarget(Rotation playerRot) {
        ...
        TargetPredicate predicate = ConfigStore.instance.getConfig().eventsScriptingConfig.enabled ? script : null;
    ...
    private @Nullable Rotation findRotation(LocalPlayer player, float speed, Rotation initial) {
