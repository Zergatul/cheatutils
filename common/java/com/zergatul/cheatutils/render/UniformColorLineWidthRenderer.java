...
public class UniformColorLineWidthRenderer {
    ...
    private UniformColorLineWidthRenderer() {
        pipeline = RenderPipeline.builder()
                ...
                .withCull(false)
                .build();
        ubo = RenderSystem.getDevice().createBuffer(() -> "Uniform Color Line Width Renderer UBO", GpuBuffer.USAGE_UNIFORM | GpuBuffer.USAGE_COPY_DST, 24);
        bufferBuilder = new VertexBufferBuilder();
    }

    public static UniformColorLineWidthRenderer getInstance() {
        return Holder.INSTANCE;
    }

    public void begin(Matrix4f mvp, float lineWidth) {
    ...
    public void end(Color color) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            Std140Builder builder = Std140Builder.onStack(stack, 24);
            builder.putVec4(
                    color.getRed() / 255f,
                    color.getGreen() / 255f,
                    color.getBlue() / 255f,
                    color.getAlpha() / 255f);
            builder.putFloat(lineWidth);
    ...
    private void point(Vector4f v, float gradient) {
        bufferBuilder.vertexFloat4(v.x, v.y, v.z, gradient);
    }

    private static final class Holder {
        public static final UniformColorLineWidthRenderer INSTANCE = new UniformColorLineWidthRenderer();
    }
}